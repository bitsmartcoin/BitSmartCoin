BitSmart-qt: Qt4 GUI for BitSmart
===============================
...
Alternatively, install Qt Creator and open the `BitSmart-qt.pro` file.

An executable named `BitSmart-qt` will be built.
...
A warning for people using the *static binary* version of BitSmart on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of BitSmart is linked against libdb5.3.
...
installed causes BitSmart-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
