...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #BitSmartTEST2\r");
            Send(hSocket, "WHO #BitSmartTEST2\r");
        } else {
            // randomly join #BitSmart00-#BitSmart05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #BitSmart%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BitSmart%02d\r", channel_number).c_str());
        }
